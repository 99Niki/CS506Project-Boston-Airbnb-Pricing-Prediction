...
#### macOS: XGBoost OpenMP Error (Apple Silicon)

If `make run` fails with an XGBoost OpenMP/libomp error on an Apple Silicon Mac (M1/M2/M3), the issue is likely an architecture mismatch. The Intel Homebrew (`/usr/local`) may have installed an x86_64 `libomp`, but your Python virtualenv is arm64 and requires the Apple Silicon version.

**Fix:** Install `libomp` via the arm64 Homebrew explicitly:
...
# should expect arm64, not x86_64
```
...
If you only have one Homebrew installed (the default for new Apple Silicon setups), `brew install libomp` should work fine.
...
We observed that some categories represent subregions or very small areas (_South Boston Waterfront_, _Leather District_, _Longwood Medical Area_). To reduce sparsity and improve model robustness, we grouped these into their larger, more representative neighborhoods based on geographic proximity and domain knowledge (e.g., _South Boston Waterfront → South Boston_, _Leather District → Downtown_, _Longwood Medical Area → Fenway_).
